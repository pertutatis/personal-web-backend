...
export class PostgresBlogArticleRepository implements BlogArticleRepository {
  ...
  async findAll(): Promise<BlogArticle[]> {
    ...
    const articlesResult =
      await this.articlesConnection.execute<ArticleRow>(articlesQuery)
    ...
    if (allBookIds.length > 0) {
      const booksQuery = `
        ...
        ORDER BY b.title;
      `
      const booksResult = await this.booksConnection.execute<BookRow>(
        booksQuery,
        [allBookIds],
      )
  ...
  async findBySlug(slug: string): Promise<BlogArticle | null> {
    ...
    const articleResult = await this.articlesConnection.execute<ArticleRow>(
      articleQuery,
      [slug],
    )
