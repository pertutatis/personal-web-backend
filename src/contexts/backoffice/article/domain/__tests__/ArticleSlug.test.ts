...
describe('ArticleSlug', () => {
  describe('constructor', () => {
    ...
    it('should throw error with invalid slug format', () => {
      expect(() => ArticleSlugMother.withInvalidFormat()).toThrow(
        ArticleSlugInvalid,
      )
    })

    it('should throw error with invalid dash positions', () => {
      expect(() => ArticleSlugMother.withInvalidDashes()).toThrow(
        ArticleSlugInvalid,
      )
    })
  ...
  describe('fromTitle', () => {
    ...
    it('should handle Spanish characters', () => {
      ...
      expect(slug.value).toBe(
        'como-implementar-patrones-de-diseno-en-javascript',
      )
    })
    ...
    it('should throw error when slug exceeds maximum length', () => {
      expect(() => ArticleSlugMother.tooLong()).toThrow(
        ArticleSlugLengthExceeded,
      )
    })

    it('should throw error for invalid characters only', () => {
      expect(() => ArticleSlugMother.withInvalidCharacters()).toThrow(
        ArticleSlugInvalid,
      )
    })
  ...
  describe('common behavior', () => {
    it('should convert to primitives correctly', () => {
      const slug = ArticleSlugMother.fromValidSlug()
      expect(slug.toString()).toBe(
        '10-clean-code-principles-every-developer-should-follow',
      )
      expect(slug.toJSON()).toBe(
        '10-clean-code-principles-every-developer-should-follow',
      )
    })
