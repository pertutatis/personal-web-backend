...
module.exports = async () => {
  try {
    ...
    execSync('docker compose exec -T blog-db psql -U postgres -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname IN (\'test_blog\')"');
    // console.info('✅ Active connections terminated');

    // Clean up test databases
    // console.info('Cleaning up test databases...');
    execSync('docker compose exec -T blog-db psql -U postgres -c "DROP DATABASE IF EXISTS test_blog"');
